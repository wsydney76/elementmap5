...
namespace wsydney76\elementmap5;
...
use wsydney76\elementmap5\models\Settings;
use wsydney76\elementmap5\services\Renderer;
use yii\base\Event;
 ...
 */
class ElementMap5 extends Plugin
{
    /**
     * @inheritdoc
    ...
    public function init()
    {
        ...
        Event::on(
            ...
            $event->rules['elementmap-getrelations/<site:.*>/<class:.*>/<id:[\d]+>'] = 'elementmap5/map/map';
        });
...
//            );
        ...
        Event::on(Asset::class, Element::EVENT_DEFINE_ATTRIBUTE_HTML, [$this, 'getTableAttributeHtml']);
...
//        Event::on(Category::class, Element::EVENT_DEFINE_ATTRIBUTE_HTML, [$this, 'getTableAttributeHtml']);
        ...
        Event::on(Entry::class, Element::EVENT_DEFINE_ATTRIBUTE_HTML, [$this, 'getTableAttributeHtml']);
...
//        Event::on(Product::class, Element::EVENT_DEFINE_ATTRIBUTE_HTML, [$this, 'getTableAttributeHtml']);
    ...
    {
        $event->tableAttributes['elementMap_incomingReferenceCount'] = ['label' => craft::t('elementmap5', 'References From (Count)')];
        $event->tableAttributes['elementMap_outgoingReferenceCount'] = ['label' => craft::t('elementmap5', 'References To (Count)')];
        $event->tableAttributes['elementMap_incomingReferences'] = ['label' => craft::t('elementmap5', 'References From')];
        $event->tableAttributes['elementMap_outgoingReferences'] = ['label' => craft::t('elementmap5', 'References To')];
    }

    /**
     * Handler for the Element::EVENT_DEFINE_ATTRIBUTE_HTML event.
     */
    public function getTableAttributeHtml(DefineAttributeHtmlEvent $event)
    {
        if ($event->attribute === 'elementMap_incomingReferenceCount') {
            $event->handled = true;
            $entry = $event->sender;
            $elements = $this->renderer->getIncomingElements($entry, $entry->site->id);
            $event->html = Craft::$app->view->renderTemplate('elementmap5/_table', ['elements' => count($elements)]);
            ...
            $entry = $event->sender;
            $elements = $this->renderer->getIncomingElements($entry, $entry->site->id);
            $event->html = Craft::$app->view->renderTemplate('elementmap5/_table', ['elements' => $elements]);
    ...
    {
        return Craft::$app->view->renderTemplate('elementmap5/_map', ['element' => $element, 'class' => $class]);
    }
