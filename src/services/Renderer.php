...
namespace wsydney76\elementmap5\services;
...
use wsydney76\elementmap5\ElementMap5;
...
// Constants
    ...
    private function getRelationships(array $elementIds, int $siteId, bool $getSources)
    {
        ...
        if (!ElementMap5::getInstance()->getSettings()->showAllSites) {
            if (!$getSources) {
                $conditions[] = [
                    'or',
                    ['sourceSiteId' => null],
    ...
    private function getEntryElements($elementIds, $siteId)
    {
        ...
        foreach ($elements as $element) {
            ...
            if ($element->isProvisionalDraft) {
                $text .= ", " . craft::t('elementmap5', 'Provisional Draft');
                $user = User::findOne($element->creatorId);
                if ($user) {
                    $text .= ", " . $user->username;
                }
            } elseif ($element->getIsDraft()) {
                $text .= ", " . craft::t('elementmap5', 'Draft');
            }
